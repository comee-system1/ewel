...
if($five == "anq"){
	...
	echo ",あなたのチームの全員が集まって話す機会は、月に何回くらいありますか。";
...
//s	echo ",あなたのチームのメンバーは、何人いますか。（上司・リーダー）を除いた数を半角整数値でお答えください。";
	...
	if(count($tlist[ 'ans' ])){
		...
		foreach($tlist[ 'ans' ] as $key=>$val){
			...
			echo mb_convert_encoding($val[ 'ans125' ],"SJIS-WIN","UTF-8").",";
			echo mb_convert_encoding($obj->emp2[$val[ 'ans4' ]],"SJIS-WIN","UTF-8").",";
			echo mb_convert_encoding($obj->emp2[$val[ 'ans5' ]],"SJIS-WIN","UTF-8").",";
