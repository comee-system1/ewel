<?PHP
	...
	//行動価値検査1
	//---------------------------------------------------
	include_once("./init/rowData/dev_data_ta.php");
	include_once("./init/rowData/raw_data_ta.php");
	include_once("./lib/keisan/functionBA.php");
	...
	while( $line = $db->fgetcsv_reg($fp, 1000)){
		if($no >= 4){
			...
			//データ計算処理
			//---------------------------------
			list($row,$lv,$standard_score,$dev_number) = BA($line,$row2,$raw_data,$dev_data);
