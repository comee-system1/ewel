<?PHP
   ini_set( 'display_errors', 0 );
	...
	//棒グラフ画像作成
	//----------------------------------------------
	$img1 = "./images/pdf/pdf34/img".$id.".jpg";
	$st_score2 = substr(round($st_score),1,1)*9;
	...
	$img_color = imagecolorallocate($im, 176, 196, 222);
...
//	imagedestroy($im);
        ...
        //重み付け画像
	$img1w = "./images/pdf/pdf34/img".$id."_w.jpg";
        
	$devw = $testdata[ 'score' ];
	$levelStr = $testdata[ 'level' ];
	$devw2 = substr(round($devw),1,1)*9;
	...
	$im = imagecreatetruecolor($wid, 15);
	$img_color = imagecolorallocate($im, 255, 127, 80);
	//$gray      = imagecolorallocate($im, 169, 169, 169);
