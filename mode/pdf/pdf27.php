<?PHP
	...
	//--------------------------------
	//PDF個人情報入力
	$make->makePdfKozin($pdf,$testdata,27);
...
*/	
	...
	$pdf->SetFillColor(0, 0, 255);
	$pdf->SetTextColor(255,255,255);
	$pdf->Cell(60, 6, "[1] 4領域の重要度", 1, 1, 'TBLR', 1);
	$pdf->Ln(2);
	...
	//横線
	$e = 8;
	$pdf->Rect(60, 73-$e, 142,0, 'D');
	$pdf->Rect(60, 79-$e, 142,0, 'D');
	$pdf->Rect(60, 85-$e, 142,0, 'D');
	$pdf->Rect(60, 91-$e, 142,0, 'D');


	//縦線
	$pdf->SetDrawColor(204,204,204);
	$pdf->Rect(62,  67-$e, 0,24, 'D');
	$pdf->Rect(85,  67-$e, 0,24, 'D');
	$pdf->Rect(108, 67-$e, 0,24, 'D');
	$pdf->Rect(131, 67-$e, 0,24, 'D');
	$pdf->Rect(154, 67-$e, 0,24, 'D');
	$pdf->Rect(177, 67-$e, 0,24, 'D');
	$pdf->Rect(200, 67-$e, 0,24, 'D');
	$pdf->SetDrawColor(102,102,102);
	$pdf->Rect(202, 67-$e, 0,24, 'D');
	...
	$pdf->Text(61,  65-$e, "0%");
	$pdf->Text(84,  65-$e, "10%");
	$pdf->Text(107, 65-$e, "20%");
	$pdf->Text(130, 65-$e, "30%");
	$pdf->Text(153, 65-$e, "40%");
	$pdf->Text(176, 65-$e, "50%");
	$pdf->Text(198, 65-$e, "60%");
	
	$pdf->Image($vf_img_bar,  62, 68.5-$e);
	$pdf->Image($vf_img_bar2, 62, 74.5-$e);
	$pdf->Image($vf_img_bar3, 62, 80.5-$e);
	$pdf->Image($vf_img_bar4, 62, 86.5-$e);
	...
	$pdf->SetFillColor(0, 0, 255);
	$pdf->SetTextColor(255,255,255);
	$pdf->Cell(60, 6, "[2] 12要素の重要度", 1, 1, 'TBLR', 1);
	$pdf->Ln(1);
	
	$pdf->Image($vfimg_main,50,112-$e);
	$pdf->Rect(10, 100-$e, 192, 130, 'D');
	$pdf->SetTextColor(0,0,0);
	$pdf->SetFontSize(8);
	$vfimg_waku = "./images/vf_waku.gif";
	$pdf->Image($vfimg_waku,42.5,109.5-$e);

	$pdf->Text(100, 121-$e, "30%");
	$pdf->Text(100, 129-$e, "25%");
	$pdf->Text(100, 137-$e, "20%");
	$pdf->Text(100, 145-$e, "15%");
	$pdf->Text(100, 153-$e, "10%");
	$pdf->Text(100, 161-$e, "5%");
	$pdf->Text(100, 169-$e, "0%");
	...
	if(array_key_exists("w1",$vfsix)){
		$pdf->SetXY(90.0, 109-$e);
		$pdf->SetFillColor(162, 199, 255);
		$pdf->SetDrawColor(128, 128, 128);
		$pdf->Rect(90.0, 107-$e, 35.0, 8.0, 'DF');
	}
	$pdf->SetXY(90,107-$e);
	...
	if(array_key_exists("w2",$vfsix)){
		$pdf->SetXY(129.0, 120-$e);
		$pdf->SetFillColor(162, 199, 255);
		$pdf->SetDrawColor(128, 128, 128);
		$pdf->Rect(127.0, 120-$e, 30.0, 9.0, 'DF');
	}
	$pdf->SetXY(127.0, 121-$e);
	$pdf->MultiCell(30, 4, $gmsg2,0,"C");

	//自己肯定力
	if(array_key_exists("w3",$vfsix)){
		$pdf->SetXY(144.0, 137-$e);
		$pdf->SetFillColor(162, 199, 255);
		$pdf->SetDrawColor(128, 128, 128);
		$pdf->Rect(142.0, 135-$e, 30.0, 8.0, 'DF');
	}
	$pdf->SetXY(142.0, 137-$e);
	$pdf->MultiCell(30, 4, $gmsg3,0,"C");

	//コントロール＆アチーブメント力
	if(array_key_exists("w4",$vfsix)){
		$pdf->SetXY(156.0, 166-$e);
		$pdf->SetFillColor(162, 199, 255);
		$pdf->SetDrawColor(128, 128, 128);
		$pdf->Rect(156.0, 164-$e, 30.0, 10.0, 'DF');
	}
	$pdf->SetXY(156.0, 166-$e);
	...
	if(array_key_exists("w5",$vfsix)){
		$pdf->SetXY(143.0, 194-$e);
		$pdf->SetFillColor(162, 199, 255);
		$pdf->SetDrawColor(128, 128, 128);
		$pdf->Rect(143.0, 192-$e, 30.0, 10.0, 'DF');
	}
	$pdf->SetXY(143.0, 193-$e);
	$pdf->MultiCell(30, 4, $gmsg5,0,"C");

	//ポジティブ思考力
	if(array_key_exists("w6",$vfsix)){
		$pdf->SetXY(129.0, 210-$e);
		$pdf->SetFillColor(162, 199, 255);
		$pdf->SetDrawColor(128, 128, 128);
		$pdf->Rect(127.0, 208-$e, 30.0, 10.0, 'DF');
	}
	$pdf->SetXY(127.0, 209-$e);
	$pdf->MultiCell(30, 4, $gmsg6,0,"C");


	//対人共感力
	if(array_key_exists("w7",$vfsix)){
		$pdf->SetXY(91.0, 220-$e);
		$pdf->SetFillColor(162, 199, 255);
		$pdf->SetDrawColor(128, 128, 128);
		$pdf->Rect(91.0, 218-$e, 30.0, 10.0, 'DF');
	}
	$pdf->SetXY(91.0, 220-$e);
	...
	if(array_key_exists("w8",$vfsix)){
		$pdf->SetXY(55.0, 210-$e);
		$pdf->SetFillColor(162, 199, 255);
		$pdf->SetDrawColor(128, 128, 128);
		$pdf->Rect(55.0, 208-$e, 30.0, 10.0, 'DF');
	}
	$pdf->SetXY(55.0, 210-$e);
	...
	if(array_key_exists("w9",$vfsix)){
		$pdf->SetXY(36.0, 194-$e);
		$pdf->SetFillColor(162, 199, 255);
		$pdf->SetDrawColor(128, 128, 128);
		$pdf->Rect(36.0, 192-$e, 30.0, 10.0, 'DF');
	}
	$pdf->SetXY(36.0, 194-$e);
	...
	if(array_key_exists("w10",$vfsix)){
		$pdf->SetXY(26.0, 166-$e);
		$pdf->SetFillColor(162, 199, 255);
		$pdf->SetDrawColor(128, 128, 128);
		$pdf->Rect(26.0, 164-$e, 30.0, 10.0, 'DF');
	}
	$pdf->SetXY(26.0, 166-$e);
	...
	if(array_key_exists("w11",$vfsix)){
		$pdf->SetXY(38.0, 137-$e);
		$pdf->SetFillColor(162, 199, 255);
		$pdf->SetDrawColor(128, 128, 128);
		$pdf->Rect(38.0, 135-$e, 30.0, 10.0, 'DF');
	}
	$pdf->SetXY(38.0, 137-$e);
	...
	if(array_key_exists("w12",$vfsix)){
		$pdf->SetXY(55.0, 120-$e);
		$pdf->SetFillColor(162, 199, 255);
		$pdf->SetDrawColor(128, 128, 128);
		$pdf->Rect(55.0, 118-$e, 30.0, 8.0, 'DF');
	}
	$pdf->SetXY(55.0, 120-$e);
	...
	$pdf->SetFontSize(10);
	$pdf->Rect(20, 107-$e, 28, 10, 'D');
	$pdf->Rect(21, 108-$e, 26, 8, 'D');
	$pdf->Text(25, 113-$e, "対人影響力");

	$pdf->Rect(165, 107-$e, 28, 10, 'D');
	$pdf->Rect(166, 108-$e, 26, 8, 'D');
	$pdf->Text(170, 113-$e, "自己認知力");

	$pdf->Rect(20, 217-$e, 28, 10, 'D');
	$pdf->Rect(21, 218-$e, 26, 8, 'D');
	$pdf->Text(25, 223-$e, "対人認知力");

	$pdf->Rect(165, 217-$e, 28, 10, 'D');
	$pdf->Rect(166, 218-$e, 26, 8, 'D');
	$pdf->Text(170, 223-$e, "自己安定力");

	$pdf->SetXY(10,232-$e);
	$pdf->SetFillColor(0, 0, 255);
	$pdf->SetTextColor(255,255,255);
	$pdf->Cell(60, 6, "[3] 重要度の高い上位6要素", 1, 1, 'TBLR', 1);
	$pdf->Ln(2);
	...
	$pdf->SetTextColor(0,0,0);
	$pdf->SetFontSize(9);
	$msg = "この検査は、強制選択法という手法を用い、組織で仕事をしていく上で 必要な12要素を順位付けしています。";
	...
	$pdf->SetFillColor(255, 255, 0);
	$pdf->SetXY(20.05,116-$sx);
	$pdf->Cell(50, 8, "２．自己安定力", 1, 0, 'L', 1);
	$pdf->SetFillColor(255, 255, 255);
	$pdf->Cell(130, 8, "自分をコントロールする力", 1, 1, 'L', 1);
	$pdf->Text(13,114-$sx, "対");
	$pdf->Text(13,117-$sx, "自");
	$pdf->Text(13,120-$sx, "分");
	...
	$pdf->SetFillColor(255, 255, 0);
	$pdf->SetXY(20.05,132-$sx);
	$pdf->Cell(50, 8, "４．対人影響力", 1, 0, 'L', 1);
	$pdf->SetFillColor(255, 255, 255);
	$pdf->Cell(130, 8, "他者を巻き込み、組織で目標を達成する力", 1, 1, 'L', 1);
	$pdf->Text(13,130-$sx, "対");
	$pdf->Text(13,133-$sx, "他");
	$pdf->Text(13,136-$sx, "者");
	...
	$pdf->SetFillColor(255, 255, 0);
	$pdf->SetXY(20.05,172-$sx);
	
	$msg3 = preg_replace("/\\n/","",$msg3);
	$pdf->Cell(50, 8, "　".$msg3, 1, 0, 'L', 1);
	$pdf->SetFillColor(255, 255, 255);
	$pdf->Cell(130, 8, "自分自身に価値があると自信を持とうとすること", 1, 1, 'L', 1);
	$pdf->Text(13.5,162-$sx, "１");
	$pdf->Text(13,165-$sx, "自");
	$pdf->Text(13,168-$sx, "己");
	$pdf->Text(13,171-$sx, "認");
	$pdf->Text(13,174-$sx, "知");
	$pdf->Text(13,177-$sx, "力");
	$pdf->Ln(2);
	...
	$pdf->SetFillColor(255, 255, 0);
	$pdf->SetXY(20.05,198-$sx);
	$msg6 = preg_replace("/\\n/","",$msg6);
	$pdf->Cell(50, 8, "　".$msg6, 1, 0, 'L', 1);
	$pdf->SetFillColor(255, 255, 255);
	$pdf->Cell(130, 8, "新しい環境や、望ましくない状況の変化に対して、柔軟で楽観的に適応すること", 1, 1, 'L', 1);
	$pdf->Text(13.5,187-$sx, "２");
	$pdf->Text(13,190-$sx, "自");
	$pdf->Text(13,193-$sx, "己");
	$pdf->Text(13,196-$sx, "安");
	$pdf->Text(13,199-$sx, "定");
	$pdf->Text(13,202-$sx, "力");
	...
	$pdf->SetFillColor(255, 255, 0);
	$pdf->SetXY(20.05,224-$sx);
	$msg9 = preg_replace("/\\n/","",$msg9);
	$pdf->Cell(50, 8, "　".$msg9, 1, 0, 'L', 1);
	$pdf->SetFillColor(255, 255, 255);
	$pdf->Cell(130, 8, "他者の利益のために自発的・意図的に行動すること", 1, 1, 'L', 1);
	$pdf->Text(13.5,212-$sx, "３");
	$pdf->Text(13,215-$sx, "対");
	$pdf->Text(13,218-$sx, "人");
	$pdf->Text(13,221-$sx, "認");
	$pdf->Text(13,224-$sx, "知");
	$pdf->Text(13,227-$sx, "力");
	...
	$pdf->SetFillColor(255, 255, 0);
	$pdf->SetXY(20.05,250-$sx);
	$msg12 = preg_replace("/\\n/","",$msg12);
	$pdf->Cell(50, 8, "　".$msg12, 1, 0, 'L', 1);
	$pdf->SetFillColor(255, 255, 255);
	$pdf->Cell(130, 8, "仲間との良好な関係を保つこと", 1, 1, 'L', 1);
	$pdf->Text(13,239-$sx, " ４");
	$pdf->Text(13,242-$sx, "対");
	$pdf->Text(13,245-$sx, "人");
	$pdf->Text(13,248-$sx, "影");
	$pdf->Text(13,251-$sx, "響");
	$pdf->Text(13,254-$sx, "力");
	$pdf->Ln();
	$pdf->SetFontSize(7);
