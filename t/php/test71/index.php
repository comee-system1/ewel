...
require_once(D_PATH_HOME."t/lib/include_ELANS2.php");
...
$where[ 'type'      ] = $first;
//受検時間の取得
$times = $elan->getTime($where);
$where[ 'test_id' ] = $times[ 'id' ];
$answerkey = $elan->getEa($where);
...
if($_REQUEST[ 'next' ]){
	...
	//
	//---------------------------------------------
	$result = $elan->setEaRst($clum,$where);

}

//回答データ取得
$answer = $elan->getEa($where);
//検査結果回答一覧画面
if($pager == 11 || $fin){
	$temp = "answerList";
}
