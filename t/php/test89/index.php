...
if ($_REQUEST[ 'next' ]) {
    ...
    if (isset($_REQUEST[ 'ans' ]) && count($_REQUEST[ 'ans' ]) > 0) {
...
if (!$_REQUEST[ 'pageBack' ]) {
    if($_REQUEST[ 'type' ] == "subordinate"){
        ...
        if ($pager == 3) {
            ...
            $total = (int)$result[ 'ans18' ]+(int)$result[ 'ans19' ]+(int)$result[ 'ans20' ]+(int)$result[ 'ans21' ];
            if ($total != 100) {
                $err[] = "設問8に不備があります。";
            }
        ...
        if ($pager == 2) {
            ...
            if (strlen($result[ 'ans11' ]) == 0) {
                $err[] = "設問6-4が未回答です。";
            }
            if (strlen($result[ 'ans12' ]) == 0) {
                $err[] = "設問6-5が未回答です。";
            }
        ...
        if ($pager == 3) {
            ...
            $total = $result[ 'ans17' ]+$result[ 'ans18' ]+$result[ 'ans19' ]+$result[ 'ans20' ];
            if ($total != 100) {
                $err[] = "設問10に不備があります。";
            }
...
if(
       ...
       // if($pager == 10 && $_REQUEST[ 'ans' ][2] == 0 ) $edit[ 'edit' ][ 'endtime' ] = date("Y-m-d H:i:s");
		...
		$tdetail = $mem;
		$s_day  = preg_split( '/\-/', $tdetail['exam_date'] );
		$s_time = preg_split( '/\:/', $tdetail['start_time'] );
