<?PHP
class NL3method extends method{
	...
	public function getEa($where){
		...
		$sql .= " sec.* ";
		$sql .= " FROM ";
		$sql .= " nl3_member as dm";
		$sql .= " INNER JOIN (SELECT * FROM nl3_sec ) as sec ON sec.mv_id=dm.id";
		$sql .= " WHERE ";
	...
	public function getScore($where){
		...
		$sql .= " ds.* ";
		$sql .= " FROM ";
		$sql .= " nl3_member as dm LEFT JOIN nl3_sec as ds ON dm.id=ds.mv_id";
		$sql .= " WHERE ";
		$sql .= " dm.test_id=".$test_id." AND ";
	...
	public function setResult($soten,$hensa,$mv_id){
		...
		if($row){
			...
			$sql = "";
			$sql = "INSERT INTO nl3_score ";
			$sql .= " (";
			$sql .= " mv_id".$scline;
			$sql .= $stline;
			$sql .= " ,regist_ts";
			$sql .= " )VALUES(";
			$sql .= $mv_id.$scores.$sotens;
			$sql .= ",'".$date."'";
			$sql .= ")";
