...
class ELANmethod extends method{
	...
	public function getEa($where){
		...
		$sql .= " sec.* ";
		$sql .= " FROM ";
		$sql .= " elan6_member as dm";
		$sql .= " INNER JOIN (SELECT * FROM elan6_sec ) as sec ON sec.elan_id=dm.id";
		$sql .= " WHERE ";
