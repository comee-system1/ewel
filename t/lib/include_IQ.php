...
class mathmethod extends method{
	...
	public function getEa($where){
		...
		$sql .= " FROM ";
		$sql .= " iq_member as m";
		$sql .= " INNER JOIN (SELECT * FROM iq_sec ) as sec ON sec.iq_id=m.id";
		$sql .= " WHERE ";
...
//		$sql .= " id = ".$id;
        ...
        public function setNowPage($where,$now=0){
               $testgrp_id = $where[ 'testgrp_id' ];
               $test_id = $where[ 'test_id' ];
               $exam_id = $where[ 'exam_id' ];
               $sql = ""
                       . " UPDATE iq_member SET "
                       . " nowpage=".$now.""
                       . " WHERE "
