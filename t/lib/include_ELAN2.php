...
class ELANmethod extends method{
	...
	public function getEa($where){
		...
		$sql .= " sec.* ";
		$sql .= " FROM ";
		$sql .= " elan2_member as dm";
		$sql .= " INNER JOIN (SELECT * FROM elan2_sec ) as sec ON sec.elan_id=dm.id";
		$sql .= " WHERE ";
