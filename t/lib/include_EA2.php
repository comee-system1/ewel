<?PHP
class EA2method extends method{
	...
	public function getEa($where){
		...
		$sql .= " sec.* ";
		$sql .= " FROM ";
		$sql .= " dp_sec_member as dm";
		$sql .= " INNER JOIN (SELECT * FROM dp_sec_secA ) as sec ON sec.dp_id=dm.id";
		$sql .= " WHERE ";
	...
	public function getScore($where){
		...
		$sql .= " ds.* ";
		$sql .= " FROM ";
		$sql .= " dp_sec_member as dm LEFT JOIN dp_sec_secA as ds ON dm.id=ds.dp_id";
		$sql .= " WHERE ";
		$sql .= " dm.test_id=".$test_id." AND ";
