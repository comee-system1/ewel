<?PHP
class SA2method extends method{
	...
	public function getEa($where){
		...
		$sql .= " sec.* ";
		$sql .= " FROM ";
		$sql .= " mv2_member as dm";
		$sql .= " INNER JOIN (SELECT * FROM mv2_sec ) as sec ON sec.mv_id=dm.id";
		$sql .= " WHERE ";
	...
	public function getScore($where){
		...
		$sql .= " ds.* ";
		$sql .= " FROM ";
		$sql .= " mv2_member as dm LEFT JOIN mv2_sec as ds ON dm.id=ds.mv_id";
		$sql .= " WHERE ";
		$sql .= " dm.test_id=".$test_id." AND ";
