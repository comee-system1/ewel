...
class mathmethod extends method{
	...
	public function getEa($where){
		...
		$sql .= " INNER JOIN (SELECT * FROM math_sec ) as sec ON sec.math_id=m.id";
		$sql .= " WHERE ";
	...
	public function setResult($list,$mathid){
		...
		$sql = "";
		$sql = "SELECT count(id) as cnt FROM math_score ";
		$sql .= " WHERE ";
		...
		$haku_hensa    = $list['haku_hensa'     ];
		$bunseki_hensa = $list['bunseki_hensa'  ];
		$sentaku_hensa = $list['sentaku_hensa'  ];
		$yosoku_hensa  = $list['yosoku_hensa'   ];
		$hyogen_hensa  = $list['hyogen_hensa'   ];

		$level         = $list[ 'level'      ];

		if(!$rst || $rst[ 'cnt' ] == 0){
			$sql = "";
			$sql = " INSERT INTO math_score ";
			$sql .= "(";
			$sql .= " math_id,haku_yoso,bunseki_yoso,sentaku_yoso,yosoku_yoso,hyogen_yoso";
			$sql .= ",haku_hensa,bunseki_hensa,sentaku_hensa,yosoku_hensa,hyogen_hensa";
			...
			$sql .= " ,sogo_yoso=".$sogo;
			$sql .= " ,haku_hensa=".$haku_hensa;
			$sql .= " ,bunseki_hensa=".$bunseki_hensa;
			$sql .= " ,sentaku_hensa=".$sentaku_hensa;
			$sql .= " ,yosoku_hensa=".$yosoku_hensa;
			$sql .= " ,hyogen_hensa=".$hyogen_hensa;
