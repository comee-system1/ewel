<?PHP
class BAGmethod extends method{
...
//初回で解答を0にする処理を削除
	...
	public function getEa($where){
		...
		$sql .= " sec.* ";
		$sql .= " FROM ";
		$sql .= " bag_member as dm";
		$sql .= " INNER JOIN (SELECT * FROM bag_secA ) as sec ON sec.dp_id=dm.id";
		$sql .= " WHERE ";
	...
	public function getScore($where){
		...
		$sql .= " ds.* ";
		$sql .= " FROM ";
		$sql .= " bag_member as dm LEFT JOIN bag_secA as ds ON dm.id=ds.dp_id";
		$sql .= " WHERE ";
		$sql .= " dm.test_id=".$test_id." AND ";
