...
class ELANmethod extends method{
	...
	public function getEa($where){
		...
		$sql .= " sec.* ";
		$sql .= " FROM ";
		$sql .= " elan3_member as dm";
		$sql .= " INNER JOIN (SELECT * FROM elan3_sec ) as sec ON sec.elan_id=dm.id";
		$sql .= " WHERE ";
