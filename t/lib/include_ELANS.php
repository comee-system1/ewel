...
class ELANmethod extends method{
	...
	public function getEa($where){
		...
		$sql .= " sec.* ";
		$sql .= " FROM ";
		$sql .= " elans_member as dm";
		$sql .= " INNER JOIN (SELECT * FROM elans_sec ) as sec ON sec.elan_id=dm.id";
		$sql .= " WHERE ";
