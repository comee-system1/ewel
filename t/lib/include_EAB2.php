<?PHP
class EAB2method extends method{
	...
	public function getEa($where){
		...
		$sql .= " sec.* ";
		$sql .= " FROM ";
		$sql .= " rs2_member as dm";
		$sql .= " INNER JOIN (SELECT * FROM rs2_secA ) as sec ON sec.rs_id=dm.id";
		$sql .= " WHERE ";
	...
	public function getScore($where){
		...
		$sql .= " ds.* ";
		$sql .= " FROM ";
		$sql .= " rs2_member as dm LEFT JOIN rs2_secA as ds ON dm.id=ds.rs_id";
		$sql .= " WHERE ";
		$sql .= " dm.test_id=".$test_id." AND ";
