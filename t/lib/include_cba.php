<?PHP
class cbamethod extends method{
	...
	public function getEa($where){
		...
		$sql .= " sec.* ";
		$sql .= " FROM ";
		$sql .= " cba_member as dm";
		$sql .= " INNER JOIN (SELECT * FROM cba_sec ) as sec ON sec.cba_id=dm.id";
		$sql .= " WHERE ";
