...
/*****
 * judge_loginが1の時は判定用ログイン画面にリダイレクト
 * 個別のログイン判定済の場合はリダイレクトしない
 */
if ($test ["judge_login"] == 1) {
 ...
 */
if ($firstType == 75) {
    header("Location:./cres.php?k=".$_REQUEST[ 'k' ]);
    exit();
}
...
if ($glob[0]) {
    ...
    $logo = "<img src='".D_URL."/img/".$logoName."' style='max-height:76px;' >";
}
if ($_REQUEST[ 'exam_id']) {
    $where = array();
...
if ($taFlg) {
    ...
    (count($_SESSION[ 'visit' ])) {
        ...
        ($_REQUEST[ 'login' ]) {
            //評価検査のとき
            //他のテストは受けない
            if ($firstType == 52 || $firstType == 60  || $firstType == 62 || $firstType == 67  || $firstType == 68 || $firstType == 86 || $firstType == 87 || $firstType == 89 || $firstType == 90) {
                $year  = sprintf("%d", $_REQUEST[ 'year'  ]);
                $month = sprintf("%d", $_REQUEST[ 'month' ]);
                $day   = sprintf("%d", $_REQUEST[ 'day'   ]);
                ...
                if (!$errmsg) {
                    ...
                    if ($member) {
                        ...
                        if ($firstType == 67  || $firstType == 68) {
                            $tlist = $t->getJugData2($member, $firstType);
                        } else {
                            //テスト情報の取得
...
if ($temp == "login") {
    ...
    $checkmax = $db->getLoginCheckMAX($where);
    $check = $db->getLoginCheck($where);
    $miss = 5-count($check);
    //if($_REQUEST[ 'aaa' ]){
        $five = strtotime(date("Y-m-d H:i:s",strtotime("-5 minute")));
        $accessdate = strtotime($checkmax[0][ 'accessdate' ]);
        if($five < $accessdate){
            if ($miss <= 0) {
                    $notlogin = true;
                    $errmsg = "
...
*/
    ...
    if ($_REQUEST[ 'login' ] && $errmsg) {
        ...
        /*
                ...
                $set[ 'input_id' ] = ($_REQUEST[ 'empnum'])?$_REQUEST[ 'empnum']:$_REQUEST[ 'exam_id' ];
...
if ($ua && $firstType != 52 && $firstType != 60  && $firstType != 62  && $firstType != 67   && $firstType != 68 && $firstType != 86 && $firstType != 87 && $firstType != 89 && $firstType != 90 ) {
    include_once("./template/mobile/".$temp.".php");
} elseif ($taFlg) {
    include_once("./template/tamen/".$temp.".php");
} elseif ($firstType == 52 || $firstType == 60 || $firstType == 62 || $firstType == 67 || $firstType == 68) {
    //評価検査
    include_once("./template/judge/".$temp.".php");
} elseif ($firstType == 86 || $firstType == 89) {
    include_once("./template/judge5/".$temp.".php");
} elseif ($firstType == 87 || $firstType == 90) {
