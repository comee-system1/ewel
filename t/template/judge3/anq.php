...
<?PHP } ?>
	<form action="<?=D_URL_TEST?>test/<?=$first?>/?k=<?=$_REQUEST[ 'k' ]?>&anq=<?=$_REQUEST[ 'anq' ]?>" method="POST">
		<p>先日受講した研修に関して以下の質問にお答えください。最もよくあてはまるものをひとつずつ選んでください。</p>
		<table class="table">
			<tr>
				<th>&nbsp;</th>
				<th>１．まったくあてはまらない</th>
...
<?PHP } ?>
                                    ...
                                    <p class="mg0" >5.アクションプランを十分には実行できていない方に伺います。その原因は何だと思いますか。実行できている方は「実行できている」とお書きください。</p>
		<br clear=all />
		<textarea name="ans[5]" class="txt" ><?=$anqid[ 'anq5' ]?></textarea>
                <br />
                <u ><font color='red'>この２週間の中で、</font></u>以下のような変化はありましたか。最もよくあてはまるものをひとつずつ選んでください。<br />
...
<?PHP foreach($array_ques2 as $key=>$val){ ?>
			<tr>
				<td nowrap ><?=$key?>.<?=$val?></td>
<?PHP for($i=1;$i<=5;$i++){?>
<?PHP 
...
<?PHP } ?>
                ...
                <p class="mg0" >12.この２週間のあなたやメンバー、チーム全体の変化について、具体的にお書きください。特にない場合は、「特になし」とお書きください。</p>
                <br clear=all />
