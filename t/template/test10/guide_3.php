...
?>
	<form action="<?=D_URL_TEST?>tamen/<?=$first?>/?k=<?=$_REQUEST[ 'k' ]?>" method="POST">
		<ol>
			<li class="red">各質問に対し、評価対象者について最もあてはまるものを選択してください。</li>
			<li>回答を入力しないと次ページに進めません。</li>
			<li>ブラウザの「戻る」ボタンは使えません。ページ下部の「１つ前のページに戻る」で、１ページ前に戻れます。</li>
			<li>長時間(<?=$times[ 'minute_rest' ]?>分)アクセスのないまま放置されますと、システム側より強制ログアウトされますので、 ご注意ください。その場合、再度ログインし直し、検査の最初から再度ご受検いただくことになります。</li>
